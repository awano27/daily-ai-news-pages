...
class XPostProcessor:
    ...
    def _calculate_post_score(text: str, username: str, post_time: datetime) -> float:
        ...
        if any(user in username.lower() for user in trusted_users):
            score += 2.0
        elif username and len(username) > 3:  # 実在ユーザー
            score += 1.0
...
def main():
    """メイン実行関数"""
    print("🎯 包括的AI Daily Intelligence Report Generator 開始")
    print("📊 RSS + X(Twitter) 統合型情報収集システム")
