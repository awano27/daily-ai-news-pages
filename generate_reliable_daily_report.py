...
def generate_clean_html_report(categorized_news: Dict[str, List[Dict]], 
                              executive_analysis: Dict[str, Any]) -> str:
    """クリーンで読みやすいHTMLレポート生成"""
...
<body>
    <div class="container">
        <div class="header">
            ...
            <div class="subtitle">{today} - 信頼性重視ビジネスインテリジェンス</div>
        </div>
    ...
    for category, news_list in categorized_news.items():
        ...
        for news in news_list[:display_limit]:
            ...
            html_content += f"""
                    <div class="news-meta">
                        <span>📰 {news.get('source', '不明')} ({news.get('source_tier_name', '')})</span>
    ...
    html_content += """
        ...
        <div class="footer">
            <p>🎯 Generated by Reliable AI Daily Intelligence System</p>
            <p>動作確認済み信頼ソース | 高品質フィルタリング | エグゼクティブ最適化</p>
        </div>
    </div>
</body>
</html>"""
...
def main():
    """メイン実行関数"""
    print("🎯 Reliable AI Daily Business Report Generator 開始")
    ...
    try:
        ...
        # 2. エグゼクティブサマリー生成
        if GEMINI_API_KEY:
            print("🧠 Gemini APIでサマリー生成中...")
            try:
                # Geminiを使用した高度な分析も可能だが、シンプル版を使用
        ...
        # 3. クリーンHTML生成
        html_report = generate_clean_html_report(categorized_news, executive_analysis)
        
        # 4. ファイル保存
        today_str = datetime.now().strftime('%Y%m%d')
        report_filename = f'reliable_daily_report_{today_str}.html'
        
        with open(report_filename, 'w', encoding='utf-8') as f:
            f.write(html_report)
        
        with open('reliable_daily_report_latest.html', 'w', encoding='utf-8') as f:
            f.write(html_report)
        
        print(f"✅ 信頼性重視レポート生成完了: {report_filename}")
