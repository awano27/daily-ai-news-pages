...
try:
    ...
    if not gemini_key:
        print("ログ: GEMINI_API_KEYが設定されていません")
        # 代替手段として環境変数を直接読み込み
...
def fetch_x_posts_from_sheets():
    ...
    try:
        print(f"ログ: Google Sheetsアクセス中: {sheet_url[:80]}...")
        ...
        for i, line in enumerate(lines[1:]):  # ヘッダー行をスキップ
            try:
                ...
                print(f"ログ: 行解析エラー {i}: {parse_error}")
                continue
...
def create_clean_dashboard():
    ...
    for article in articles[:25]:  # 25件まで検証に増加
        ...
        if is_valid:
            ...
            print(f"ログ: 検証OK: {article['title'][:50]}...")
        else:
            print(f"ログ: 検証NG: {article['title'][:50]}... ({reason})")
...
def main():
    """メイン処理"""
    try:
        html_content, articles_count, posts_count = create_clean_dashboard()
        
        current_time = datetime.now(timezone(timedelta(hours=9)))
        filename = f"clean_dashboard_{current_time.strftime('%Y%m%d_%H%M%S')}.html"
        
        with open(filename, 'w', encoding='utf-8') as f:
            f.write(html_content)
        
        print(f"✅ クリーンダッシュボード生成完了: {filename}")
        print(f"📊 掲載記事: {articles_count}件")
        print(f"📱 X投稿: {posts_count}件")
