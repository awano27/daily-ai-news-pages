...
# -*- coding: utf-8 -*-
"""
ソースリンク付きユーザーフレンドリーダッシュボード
Gemini API日本語要約、ソースリンク表示、視覚性改善
"""
...
def fetch_x_posts_simple():
    ...
    print(f"✅ X投稿データ取得完了: {len(dummy_posts)}件")
    return dummy_posts
...
def generate_business_insights(web_data):
    """ビジネスインサイト生成（ソースリンク付き）"""
    insights = []
    ...
    for category, articles in web_data.items():
        ...
        for article in articles[:2]:  # 各カテゴリ上位2件
            ...
            insights.append({
                ...
                'url': url,  # ソースURL追加
                'impact': '高' if 'GPT' in title or 'AI' in title or '投資' in title else '中',
                'action_required': 'AI' in title or 'OpenAI' in title or 'Google' in title
            })
    
    return insights

def generate_dashboard_with_links(analysis_file: str = None):
    """ソースリンク付きダッシュボード生成"""
    ...
    business_insights = generate_business_insights(web_data)
    rss_items = fetch_rss_feeds_simple()
    x_posts = fetch_x_posts_simple()  # 最後に配置
    ...
    <div class="container">
        ...
        <div class="stats-overview">
            ...
            <div class="stat-card">
                ...
                <div class="stat-label">話題の投稿</div>
            </div>
            <div class="stat-card">
...
"""
    ...
    if business_insights:
        ...
        for insight in business_insights:
            ...
            # タイトルをリンクにする（URLがある場合）
            title_html = insight['title']
            if insight.get('url'):
                title_html = f'<a href="{insight["url"]}" target="_blank">{insight["title"]}</a>'
            ...
            if insight.get('url'):
                html += f"""
                            <a href="{insight['url']}" target="_blank" class="source-link">
                                🔗 ソースを見る
                            </a>
                """
    ...
    if rss_items:
        ...
        for item in rss_items:
            ...
            if item.get('link'):
                title_html = f'<a href="{item["link"]}" target="_blank">{item["title"]}</a>'
...
def main():
    ...
    # ダッシュボード生成
    html = generate_dashboard_with_links(str(latest_file) if latest_file else None)
    
    # HTMLファイル保存
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_file = f"business_dashboard_with_links_{timestamp}.html"
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html)
    
    print(f"✅ ソースリンク付きダッシュボード生成完了: {output_file}")
