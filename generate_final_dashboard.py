...
def generate_final_dashboard(analysis_file: str = None):
    """最終版ダッシュボード生成（固定リンクセクション付き）"""
...
def main():
    ...
    # ダッシュボード生成
    html = generate_final_dashboard(str(latest_file) if latest_file else None)
    
    # HTMLファイル保存
    timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
    output_file = f"ai_business_dashboard_{timestamp}.html"
    
    with open(output_file, 'w', encoding='utf-8') as f:
        f.write(html)
    
    print(f"✅ 最終版ダッシュボード生成完了: {output_file}")
